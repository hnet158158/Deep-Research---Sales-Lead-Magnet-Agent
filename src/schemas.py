...
Тон и стиль: Информационный стиль (Ильяхов), но живой и человечный. 
1. Строго запрещены отглагольные существительные ('осуществление', 'формирование', 'реализация'). 
2. Запрещены академические и чиновничьи штампы ('в условиях меняющейся экономики', 'критически важно', 'фундаментальная трансформация'). Пишите простым языком, как для коллеги-предпринимателя. 
3. Используйте активный залог ('мы собрали', а не 'было собрано'). 
...
Инструкции:
- Исправьте только явные орфографические, пунктуационные и грамматические ошибки.
- Сохраните исходную структуру документа 1:1 (те же секции, порядок, заголовки).
- НЕ сокращайте и НЕ расширяйте секции более чем на 5%.
- Запрещено уменьшать суммарную длину документа; итоговый текст должен быть не короче исходного черновика.
- НЕ удаляйте факты, ссылки и числовые значения.
- НЕ объединяйте и НЕ дробите разделы.
- Чередуйте лица. Не начинайте каждый абзац со слова «Мы», используйте безличные активные конструкции (Вместо «Мы создаем» -> «Студии создают» или «Важно создавать»).
- Сохраните Markdown-форматирование; разрешены только точечные правки.
- Add placeholders where you feel I should add images с комментарием типа `Добавьте сюда изображение номер N о том-то`
- {link_rule}
...
Жесткие ограничения:
- Секция: "{section_name}".
- Сохраните заголовок и внутреннюю структуру абзацев/списков.
